...
RSpec.describe 'Sessions', type: :request do
  ...
  let(:request_login_ok) do
    post '/api/login', params: '{ "email": "testing@test.com", "password": "qwerty123" }',
                       headers: { 'CONTENT_TYPE' => 'application/json' }
  end

  let(:request_login_not_found_email) do
    post '/api/login', params: '{ "email": "not_found_email@test.com", "password": "qwerty123" }',
                       headers: { 'CONTENT_TYPE' => 'application/json' }
  end

  let(:request_login_bad_password) do
    post '/api/login', params: '{ "email": "testing@test.com", "password": "bad password" }',
                       headers: { 'CONTENT_TYPE' => 'application/json' }
  end

  let(:request_signup_ok) do
    post '/api/signup', params: '{ "keywords": ["test"], "password": "signup_password", "name": "Testing Test",
                               "email": "signup@test.com" }', headers: { 'CONTENT_TYPE' => 'application/json' }
  end

  let(:request_signup_without_name) do
    post '/api/signup', params: '{ "keywords": ["test"], "password": "signup_password",
                               "email": "signup@test.com" }', headers: { 'CONTENT_TYPE' => 'application/json' }
  end

  let(:request_signup_without_keywords) do
    post '/api/signup', params: '{ "password": "signup_password", "name": "Testing Test",
                               "email": "signup@test.com" }', headers: { 'CONTENT_TYPE' => 'application/json' }
  end
